...
(defun magithub-settings--value-or (variable default &optional accessor)
  (declare (indent 2))
  (let ((args (list variable)))
    (unless (magit-toplevel)
      (push "--global" args))
    (let ((val (apply #'magit-get args)))
      (if (and val accessor)
          (apply accessor args)
        (or val default)))))
...
(defvar magithub-confirmation
  ...
  '((pre-submit-pr                        short "You are about to create a pull request to merge branch `%s' into %s:%s; is this what you wanted to do?")
    ...
    (pair-repo-with-directory             short "Associate %s with %s?")
    (pair-repo-with-directory-overwrite   short "%s is already associated with %s.  Overwrite?")
    (pair-repo-with-directory-interactive short "There's no directory associated with %s.  Would you like to set one?"))
...
might belong in Magit, but we'll see how it goes."
  (let ((spec (alist-get action magithub-confirmation))
    ...
    (unless spec
      (magithub-error (format "No confirmation settings for %S" action)))
    (unless (= 2 (length spec))
      (magithub-error (format "Spec for %S must have 2 members: %S" action) spec))
